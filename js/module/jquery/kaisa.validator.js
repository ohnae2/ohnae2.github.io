	...
	kaisa.validator = {
		...
		help : function(settings){
			...
			if(posX == 0 && posY == 26){ //hidden input 해당할때
				kaisa.alert.open({message : settings.message});
				return;
			}
		...
		submit : function(settings){
			...
			settings.element.submit(function(e){
				e.preventDefault();
				if(kaisa.validator.validate(settings.element[0])){
		...
		validate: function(form, vrs /* Optional */){
			...
			$(form).find(':input[data-valid]').each(function(i){
				...
				var rs = kaisa.validator.parseInlineVrs(vrs);
				for(var n in rs){
					if(n != 'label'){
						var param = rs[n];
						if(param == false){
						...
						var result = kaisa.validator.rules[n].check.call(kaisa.validator, j.val(), this, param);
						if(result != true){
							var message = '\'' + rs['label'] + '\' ' + kaisa.validator.formatMessage(kaisa.validator.rules[n].msg, param);
							if(overMessage){
								message = overMessage;
							}
							kaisa.validator.help({
								input : j,
								message : message
							});
							sucess = false;
		...
		parseInlineVrs: function(vrs){
			...
			for(var i = 0; i < ss.length; ++i){
				...
				if(r){
					...
					if(kaisa.validator.rules[ruleName] && kaisa.validator.rules[ruleName].argc > 1){
						var ruleValueCount = kaisa.validator.rules[ruleName].argc;
						// XXX 이거 뭐냐 이상해
		...
		getByteLength: function(value, element){
			...
			case 'select':
				return $('option:selected', element).length;
			case 'input':
				if( kaisa.validator.checkable( element) ){
					return $(element).filter(':checked').length;
				}
			}
			var ibyte = 0;
		...
		depend: function(param, element){
			return kaisa.validator.dependTypes[typeof param]
				? kaisa.validator.dependTypes[typeof param](param, element)
				: true;
		},
		dependTypes: {
		...
		optional: function(element){
			return !kaisa.validator.rules.required.check.call(kaisa.validator, $.trim(element.value), element);
		},
		rules: {
			required: {
				argc: 1,
...
//					if( !kaisa.validator.depend(param, element) ){
//						return false;
//					}
					switch( element.nodeName.toLowerCase() ){
					case 'select':
			...
			email: {
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test(value);
				}
			},
			url: {
				argc: 1,
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(value);
				}
			},
			date: {
				argc: 1,
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || /^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$/.test(value);
				}
			},
			number: {
				argc: 1,
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(value);
				}
			},
			digit: {
				argc: 1,
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || /^\d+$/.test(value);
				}
			},
			equal: {
				argc: 1,
			...
			min: {
				...
				msg: '{0} 이상의 값을 입력하세요.',
				check: function(value, element, param){
					return kaisa.validator.optional(element) || value >= param;
				}
			},
			max: {
				argc: 1,
			...
			range: {
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || ( value >= param[0] && value <= param[1] );
				}
			},
			equallength: {
				argc: 1,
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || kaisa.validator.getLength($.trim(value), element) == param;
				}
			},
			minlength: {
				argc: 1,
			...
			rangelength: {
				...
				check: function(value, element, param){
					var length = kaisa.validator.getLength($.trim(value), element);
					return kaisa.validator.optional(element) || ( length >= param[0] && length <= param[1] );
				}
			},
			minbytes: {
				argc: 1,
				...
				check: function(value, element, param){
					return kaisa.validator.optional(element) || kaisa.validator.getByteLength($.trim(value), element) >= param;
				}
			},
			maxbytes: {
				argc: 1,
			...
			rangebytes: {
				...
				check: function(value, element, param){
					var length = kaisa.validator.getByteLength($.trim(value), element);
					return kaisa.validator.optional(element) || ( length >= param[0] && length <= param[1] );
				}
			},
			xss: {
