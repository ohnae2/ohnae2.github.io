 ...
 * 4.kaisa.alert & kaisa.loading & kaisa.dimmed 필요
 */
(function(){
	"use strict";
	/**
	 ...
	 * 날짜포맷 설정
	 */
	kaisa.datePicker = function(settings){
		var base = {
			...
			open : function(){ //(선택)달력열기
				...
				//레이어 열기
				this.layer.show();
				//kaisa.dimmed.open({target:'datePicker',transparent:true}); //딤투명처리
			},
			close : function(){ //(선택)달력닫기
				this.layer.hide();
				//kaisa.dimmed.close({target:'datePicker'});
			},
		...
		if(settings.id){
			kaisa.datePicker[settings.id] = settings;
		}
		...
		var process = function(obj,parent){
			...
			parent.find('td span').click(function(){
				...
				if(settings.start.date > settings.end.date && settings.dual){
					kaisa.alert.open({message:'시작 날짜는 종료 날짜 이후로 설정할 수 없습니다.'});
		...
		var setCalendar = function(obj,parent){
			...
			if(settings.layer.find('.on.disabled').length > 0){
				...
				if(settings.id == "drawDatePicker" ){
					kaisa.alert.open({message:'추첨일시는 응모기간보다 빠를 수 없습니다.다른 날짜를 선택해주세요.'});
				}else if(settings.id == "winDatePicker" ){
					kaisa.alert.open({message:'당첨 발표일시는 추첨일시보다 빠를 수 없습니다.다른 날짜를 선택해주세요.'});
				}else{
					kaisa.alert.open({message:'입력 불가능한 날짜입니다. 다른 날짜를 선택해주세요.'});
				}
			}else{
				settings.fail = false;
			}
		...
		settings.element.find('.btnDatePicker').click(function(event){
			...
			var btnWrap = ''+
			'<div class="btnWrap">'+
				'<button class="small confirm" type="button">확인</button>'+
				'<button class="small cancel" type="button">취소</button>'+
			'</div>';
			...
			endCon.find('.select_year').change(function(){
				...
				setCalendar(settings.end.date,endCon);
				if(settings.start.date > settings.end.date && settings.dual){
					kaisa.alert.open({message:'종료 날짜는 시작 날짜 이전으로 설정할 수 없습니다.'});
					settings.fail = true;
